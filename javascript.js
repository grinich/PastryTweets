...
controller.init = function () {
    ...
    setTimeout(function () { controller.addVoiceOverButton() }, 100);
...
controller.tableViewNumberOfRowsInSection = function (tableView, section) {
	...
	if (this.activeElement == "TOP_LEVEL_ITEM") {
        var length = this.currentElements.length;
        length += 1;
    ...
    if (tableView.style == PKTableViewStylePlain) {
        return this.currentElements.length
    }

    var section_header = controller.tableViewTitleForHeaderInSection(tableView, section);
    if (section_header == "") {
        return 1
    }
    for (var element_index in this.currentElements) {
        var f = this.currentElements[element_index];
...
controller.fixNavigationSize = function () {
    if (controller.navigation) {
        controller.navigation.size.height = parseInt(document.body.style.height)
    }
};
